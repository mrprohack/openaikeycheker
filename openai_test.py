...
def is_valid_api_key(api_key):
    ...
    try:
        openai.Completion.create(
            engine='davinci', prompt='Hello', max_tokens=10)
        return True
...
parser.add_argument('-f', '--api_key_file',
                    help='Path to the file containing API keys')
parser.add_argument('-o', '--output_file',
                    help='Path to save the valid API keys')
...
if api_key_file:
    ...
    if output_file:
        with open(output_file, 'w') as file:
            file.write('\n'.join(valid_api_keys))
        print(colored(
            f"Valid API keys saved to '{output_file}' and Number of valid API keys: {valid_count}.", 'blue'))
    else:
